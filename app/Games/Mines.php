...
class Mines extends ExtendedGame implements MultiplierCanBeLimited {
  ...
  public function data(): array {
    if (!Cache::has('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 0.99))) {
      $output = [];
      ...
      $calculateMultiplier = function ($mines, $diamonds) use ($nCr) {
        $payout = ($nCr(25, $diamonds) / $nCr(25 - $mines, $diamonds));
        $percent = HouseEdgeModule::get($this, 0.99);

        return $payout - ($percent / 100 * $payout);
      };
      ...
      Cache::forever('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 0.99), json_encode($output));
    }
    return json_decode(Cache::get('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 0.99)), true);
  }

  private function getGrid(Game $game, string $server_seed) {
    Log::info('following is the content of mines game_data');
    Log::info($game->data['game_data']);
    if(array_key_exists('grid', $game->data['game_data'])) {
       Log::info('Returning already generated grid!');
       return $game->data['game_data']['grid'];
    }
